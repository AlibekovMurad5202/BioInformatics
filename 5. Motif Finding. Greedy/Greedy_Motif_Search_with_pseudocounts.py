def greedy_motif_search_with_pseudocounts(DNA, k, t):
    ...
    for j in range(len(DNA[0]) - k + 1):
        motifs = [DNA[0][j:j + k]]
        for i in range(1, t):
            profile = form_profile_with_pseudocounts(motifs, k)
...
def form_profile_with_pseudocounts(motifs, k) -> dict:
    t = len(motifs)
    profile = { c: [1. / (t + 4)] * k for c in ('A', 'C', 'G', 'T') }
    for motif in motifs:
        for i in range(k):
            profile[motif[i]][i] += 1. / (t + 4)
...
if __name__ == "__main__":
    ...
    motifs = greedy_motif_search_with_pseudocounts(DNA, k, t)
    for motif in motifs:
        print(motif)